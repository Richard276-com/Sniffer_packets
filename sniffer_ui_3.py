...
Run with:  sudo python3 sniffer_tui.py   (raw-socket capture needs root)
"""

import sys, os, socket, struct, time, csv, json, copy, datetime
...
def parse_packet(no: int, raw: bytes, t0: float) -> PacketRecord:
    p = PacketRecord(no=no, timestamp=time.time()-t0, length=len(raw), raw=raw)
    if len(raw) < 14:
...
def build_detail_tree(p: PacketRecord) -> list:
    ...
    sections = []
    ts = datetime.datetime.fromtimestamp(p.timestamp if p.timestamp > 0 else time.time()).strftime("%b %d, %Y %H:%M:%S.%f")
    sections.append((
...
class CaptureThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            self.error_occurred.emit("Permission denied — please run as root:\n  sudo python3 sniffer_tui.py")
            return
        except OSError as e:
...
class SettingsDialog(QDialog):
    ...
    def _add_pages(self):
        ...
        self._interface   = combo(CAPTURE_IFACES, mw.settings.get("interface","eth0"))
        ...
        for proto, (bg, fg) in PROTO_COLORS.items():
            ...
            badge = QLabel(proto); badge.setFixedWidth(54); badge.setAlignment(Qt.AlignCenter)
        ...
        for action, key in shortcuts:
            ...
            ki = QTableWidgetItem(key); ki.setTextAlignment(Qt.AlignRight|Qt.AlignVCenter); tbl.setItem(r, 1, ki)
        p11l.addWidget(tbl)
...
class StatsDialog(QDialog):
    def __init__(self, packets: List[PacketRecord], parent=None):
        ...
        for k, v in [("Total Packets", str(total)), ("Total Bytes", f"{total_bytes:,}"), ("Protocols", str(len(counts)))]:
            ...
            vl.setAlignment(Qt.AlignCenter); kl.setAlignment(Qt.AlignCenter)
        ...
        for proto, cnt in sorted(counts.items(), key=lambda x:-x[1]):
            ...
            pi.setFont(QFont("", -1, QFont.Bold))
            tbl.setItem(r,0,pi)
            for col, val in enumerate([str(cnt), f"{proto_bytes:,}", f"{pct:.1f}%"], 1):
                item = QTableWidgetItem(val); item.setTextAlignment(Qt.AlignRight|Qt.AlignVCenter)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"{APP_NAME} — Network Packet Analyzer")
        ...
        self.settings: dict = {
            ...
            "interface": "eth0", "promiscuous": True, "monitorMode": False,
            "snapLen": 65535, "maxPackets": 10000, "autoScroll": True,
            "confirmClear": True, "timestampFmt": "Relative", "timestampPrec": 6,
            ...
            "cols": {k: True for k in ("no","time","src","dst","proto","len","info")},
            "savedFilters": [
                {"name": "HTTP only",    "filter": "http"},
                {"name": "My machine",   "filter": "ip.addr==192.168.1.1"},
                {"name": "DNS queries",  "filter": "dns"},
                {"name": "Large packets","filter": "frame.len>500"},
            ],
            "protoColors": {},
            "notifyProtos": {p: False for p in PROTO_COLORS},
        }
    ...
    def _fit_columns(self):
        hdr = self.packet_table.horizontalHeader()
    ...
    def _set_theme(self, name: str):
        ...
        self.status_bar.setStyleSheet(f"QStatusBar{{background:{self.theme['statusBg']};color:white;font-size:11pt;padding:3px 10px;}}")

    def _apply_theme_palette(self):
        t = self.theme
        pal = QPalette()
        def c(s): return QColor(s)
        pal.setColor(QPalette.Window,          c(t["window"]))
        pal.setColor(QPalette.WindowText,      c(t["text"]))
        pal.setColor(QPalette.Base,            c(t["base"]))
        pal.setColor(QPalette.AlternateBase,   c(t["altBase"]))
        pal.setColor(QPalette.ToolTipBase,     c(t["panel"]))
        pal.setColor(QPalette.ToolTipText,     c(t["text"]))
        pal.setColor(QPalette.Text,            c(t["text"]))
        pal.setColor(QPalette.Button,          c(t["btn"]))
        pal.setColor(QPalette.ButtonText,     ... (36 KB rimanenti)